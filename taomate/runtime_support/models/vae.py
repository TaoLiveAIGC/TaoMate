...
def create_vae_wrappers(
    ...
    # Keep on CPU – VAEs are only needed for periodic visualization / benchmark.
    # Use _vae_to_device() to move on-demand.
    video_decoder = video_decoder.to(dtype=dtype)
    audio_decoder = audio_decoder.to(dtype=dtype)
