...
def _build_gemma_processor(image_processor, tokenizer):
    try:
        return Gemma3Processor(image_processor=image_processor, tokenizer=tokenizer)
    except ImportError as exc:
        if "ReasoningEffort" not in str(exc):
            raise
        return SimpleNamespace(image_processor=image_processor, tokenizer=tokenizer)
...
class GemmaTextEncoder(torch.nn.Module):
    ...
    def _compute_device(self) -> torch.device:
        """Detect actual compute device, handling FSDP CPU FlatParameters.

        Under FSDP, FlatParameters reside on CPU until forward-unsharded,
        so ``next(model.parameters()).device`` misleadingly returns CPU.
        When FSDP wrapping is detected, fall back to the local CUDA device.
        """
        try:
    ...
    def encode(
        ...
        _device = self._compute_device()
        input_ids = torch.tensor([[t[0] for t in token_pairs]], device=_device)
        attention_mask = torch.tensor([[w[1] for w in token_pairs]], device=_device)
...
class GemmaTextEncoderModelBase(GemmaTextEncoder):
    """
...
def module_ops_from_gemma_root(gemma_root: str) -> tuple[ModuleOps, ...]:
    ...
    def load_processor(module: GemmaTextEncoderModelBase) -> GemmaTextEncoderModelBase:
        ...
        module.processor = _build_gemma_processor(image_processor, module.tokenizer.tokenizer)
        return module
