...
class _BasicTransformerBlock1D(torch.nn.Module):
    ...
    def forward(
        ...
        # 2. Self-Attention
        # text encoder connector runs on a replicated short prompt-token
        # sequence (~1024) on every SP rank, so it must opt out of SP a2a;
        # otherwise q gets all-to-all'd to seq*sp_size while mask stays at seq.
        attn_output = self.attn1(norm_hidden_states, mask=attention_mask, pe=pe, enable_sp=False)
