...
# Device-aware cache for freq grids to avoid CPU→GPU sync on every call
_freq_grid_device_cache: dict[tuple, torch.Tensor] = {}
...
def _get_freq_grid_on_device(
    ...
    """Get freq grid cached on the target device, avoiding repeated CPU→GPU transfers."""
    cache_key = (id(freq_grid_generator), theta, max_pos_count, inner_dim, str(device))
    cached = _freq_grid_device_cache.get(cache_key)
    if cached is not None:
        return cached
    # Generate on CPU (lru_cache) then move to target device once
    cpu_indices = freq_grid_generator(theta, max_pos_count, inner_dim)
    gpu_indices = cpu_indices.to(device=device, non_blocking=True)
    _freq_grid_device_cache[cache_key] = gpu_indices
    # Evict old entries if cache grows too large
    if len(_freq_grid_device_cache) > 20:
        oldest_key = next(iter(_freq_grid_device_cache))
        del _freq_grid_device_cache[oldest_key]
    return gpu_indices
...
def get_fractional_positions(indices_grid: torch.Tensor, max_pos: list[int]) -> torch.Tensor:
    ...
    # Vectorized: broadcast divide then permute — avoids list comprehension + stack
    # indices_grid: (B, n_pos_dims, seq_len)
    max_pos_tensor = indices_grid.new_tensor(max_pos).reshape(1, n_pos_dims, 1)
    fractional_positions = indices_grid / max_pos_tensor  # (B, n_pos_dims, seq_len)
    return fractional_positions.permute(0, 2, 1)  # (B, seq_len, n_pos_dims)
...
def generate_freqs(
    ...
    # indices should already be on the correct device (via _get_freq_grid_on_device)
    if indices.device != fractional_positions.device:
        indices = indices.to(device=fractional_positions.device, non_blocking=True)
...
def precompute_freqs_cis(
    ...
    # Use device-aware cache to get indices directly on the target device
    indices = _get_freq_grid_on_device(
        freq_grid_generator, theta, indices_grid.shape[1], dim, indices_grid.device
    )
