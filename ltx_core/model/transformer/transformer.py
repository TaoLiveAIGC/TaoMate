...
class BasicAVTransformerBlock(torch.nn.Module):
    ...
    def _apply_text_cross_attention(
        ...
        Sequence-Parallel note:
            Text ``context`` is replicated on every SP rank (text encoder
            output is broadcast, not seq-sharded). We pass
...
def apply_cross_attention_adaln(
    ...
    # Text cross-attention: context is replicated on every SP rank.
    return attn(
        attn_input,
